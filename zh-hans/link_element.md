...
| *`type`* | 此属性用于定义链接到的**内容的类型**。该属性的值应为 *[[MIME]]类型*，例如`text/html`，`text/css`等。此属性的常见用法是定义所引用样式表的类型（例如`text/css`），但是鉴于[[CSS]]是网络上使用的*唯一*样式表语言，不仅可以省略`type`属性，但实际上是现在建议的做法。它还用于`rel="preload"`链接类型，以确保浏览器仅下载其支持的文件类型。
