...
| 参数 | 说明 |
| :-- | :-- |
| `callback` | 用来测试数组的每个元素的函数。返回 `true` 表示该元素通过测试，保留该元素，`false` 则不保留。它接受以下三个参数：<br>**`element`**<br>数组中当前正在处理的元素。<br>**`index`** 可选<br>正在处理的元素在数组中的索引。<br>**`array`** 可选<br>调用了 `filter` 的数组本身。
