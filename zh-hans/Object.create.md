...
| :-- | :-- |
| `proto` | 新创建对象的原型对象。 |
| `propertiesObject` | 可选。如果没有指定为 [`undefined`](/zh-hans/webfrontend/undefined)，则是要添加到新创建对象的不可枚举（默认）属性（即其自身定义的属性，而不是其原型链上的枚举属性）对象的属性描述符以及相应的属性名称。这些属性对应[`Object.defineProperties()`](/zh-hans/webfrontend/Object.defineProperties)的第二个参数。 |
...
`Object.assign` 会把 `OtherSuperClass` 原型上的函数拷贝到 `MyClass` 原型上，使 `MyClass` 的所有实例都可用 `OtherSuperClass`
