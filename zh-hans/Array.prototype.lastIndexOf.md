TOPICS: Array.prototype.lastIndexOf
...
下例使用 `lastIndexOf` 查找到一个元素在数组中所有的索引（下标），并使用 [`push`](/zh-hans/webfrontend/Array.prototype.push) 将所有添加到另一个数组中。
...
注意，我们要单独处理`idx==0`时的情况，因为如果是第一个元素，忽略了`fromIndex`参数则第一个元素总会被查找。这不同于[`indexOf`](/zh-hans/webfrontend/Array.prototype.indexOf)方法
...
[`Number`](/zh-hans/webfrontend/Number_Object)、[`Math.floor`](/zh-hans/webfrontend/Math.floor)、
